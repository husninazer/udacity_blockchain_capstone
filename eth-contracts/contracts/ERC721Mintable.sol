...
contract Ownable {
    ...
    modifier onlyOwner() {
      require(msg.sender == _owner, "Sender should be owner");
      _;
    }
    ...
    // Public Getter for current Owner
    function getOwner
                    (
...
contract Pausable is Ownable {
    ...
    //Added functions as per requirement
    function setPause
                    (
                      bool status
                    )
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(_tokenOwner[tokenId] != to,"The address is the owner itself");
